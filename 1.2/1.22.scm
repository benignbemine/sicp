...
; with an integer n, prints n and checks to see if n is prime. If n is
; prime, the procedure prints three asterisks followed by the amount
...
; 1000; larger than 10,000; larger than 100,000; larger than 1,000,000.
...
(define (search-for-primes lb ub howMany?)
        (cond ((even? lb) (search-for-primes (+ lb 1) ub howMany?))
              ((= howMany? 0) (display " finished, enough found "))
              ((> lb ub) (display " finished, range over "))
              ((= (timed-prime-test lb) -1)
                  (search-for-primes (+ lb 2)  ub howMany?))
              (else (search-for-primes (+ lb 2)  ub (- howMany? 1)))))


(search-for-primes 10000000 1000000000 3)
